...
import { ArrowRight, FileUp, UsersRound, Waypoints } from "lucide-react";
...
export default async function ImportPage({ searchParams }: { searchParams: Promise<{ kind?: string; success?: string; error?: string }> }) {
  ...
  return <main className="content narrow-content"><PageHeader eyebrow="Data migration" title="Import records" description="Map and reconcile spreadsheet data before any records change." icon={FileUp} /><FlashMessage {...params} />{canMigrate ? <section className="migration-route"><span className="panel-icon"><Waypoints size={18} /></span><div><strong>Migrate from CutTime</strong><p>Guided, one-time cutover for a new Band Office program. Upload CutTime exports; Band Office never connects to your CutTime account.</p></div><Link className="button secondary" href="/import/cuttime">Open migration <ArrowRight size={15} /></Link></section> : null}{canImportPeople ? <section className="migration-route"><span className="panel-icon"><UsersRound size={18} /></span><div><strong>Import CutTime guardians</strong><p>Use the Guardian 1 and Guardian 2 columns already included in CutTime&apos;s member export to populate family links after a roster cutover.</p></div><Link className="button secondary" href="/import/cuttime-guardians">Import guardians <ArrowRight size={15} /></Link></section> : null}<h2 className="import-section-title">Spreadsheet imports</h2><div className="segment-tabs">{canImportPeople ? <Link className={!assets ? "active" : ""} href="/import">Students</Link> : null}{canImportAssets ? <Link className={assets ? "active" : ""} href="/import?kind=assets">Assets</Link> : null}</div>{assets ? <ImportWizard kind="assets" fields={assetFields} action={importAssetsAction} uniqueField={{ key: "schoolAssetTag", label: "Asset tag" }} /> : <ImportWizard kind="students" fields={memberFields} action={importStudentsAction} />}<p className="privacy-copy">Imports become school records. Review the dry run and keep source files in district-approved storage.</p></main>;
}
