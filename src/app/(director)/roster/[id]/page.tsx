...
import { ArrowLeft, ClipboardPlus, Pencil, Plus, UserRound, UsersRound, WalletCards } from "lucide-react";
import { notFound, redirect } from "next/navigation";
import { addGroupMembershipAction, addPersonClassificationAction, endGroupMembershipAction, linkGuardianStudentAction, unlinkGuardianStudentAction, updateAgreementAction, updatePersonAction } from "@/app/actions";
...
export default async function PersonDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ success?: string; error?: string }> }) {
  ...
  const [groups, guardianCandidates, studentCandidates] = await Promise.all([
    ...
    db.person.findMany({ where: { programId: person.programId, id: { not: person.id }, classifications: { some: { classification: PersonClassificationType.GUARDIAN } } }, orderBy: [{ lastName: "asc" }, { firstName: "asc" }] }),
    db.person.findMany({ where: { programId: person.programId, id: { not: person.id }, studentProfile: { isNot: null } }, orderBy: [{ lastName: "asc" }, { firstName: "asc" }] }),
  ]);
  ...
  return <main className="content">
    ...
    <div className="detail-grid">
      ...
      <aside className="detail-aside"><h3>Person record</h3><dl className="fact-list"><div><dt>Status</dt><dd><StatusPill value={person.status} /></dd></div>{canViewContactDetails ? <><div><dt>Email</dt><dd>{person.email ?? "Not set"}</dd></div><div><dt>Phone</dt><dd>{person.phone ?? "Not set"}</dd></div></> : null}{isStudent ? <><div><dt>Student ID</dt><dd>{person.studentProfile!.schoolStudentId ?? "Not set"}</dd></div><div><dt>Grade</dt><dd>{person.studentProfile!.grade}</dd></div></> : null}</dl>{canViewNotes && person.notes ? <div className="notes-block"><span>Director notes</span><p>{person.notes}</p></div> : null}{canViewNotes ? <p className="privacy-copy">No medical, disciplinary, or family information. Notes are included in exports.</p> : null}</aside>
    </div>
  </main>;
}
