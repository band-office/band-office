...
export default async function EmailSettingsPage({ searchParams }: { searchParams: Promise<{ success?: string; error?: string }> }) {
  ...
  return <main className="content narrow-content">
    ...
    <section className="settings-section"><div className="section-heading"><div><h2>SMTP credential</h2><p>The password is never written to BandOS records or backups.</p></div><ShieldCheck size={19} /></div><EmailCredentialControl environmentCredential={Boolean(process.env.BANDOS_SMTP_PASSWORD)} /></section>
