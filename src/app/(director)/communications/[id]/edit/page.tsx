...
export default async function EditAnnouncementPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string; success?: string }> }) {
  ...
  return <main className="content narrow-content compose-content">
    ...
    <form action={updateAnnouncementAction} className="compose-form"><input type="hidden" name="announcementId" value={id} />
      ...
      <section className="compose-section delivery-section"><div><Field label="Schedule (optional)" hint="Desktop delivery runs only while BandOS is open."><input name="scheduledAt" type="datetime-local" defaultValue={localDateTime(announcement.scheduledAt)} /></Field></div><div className="form-actions"><Link className="button secondary" href={`/communications/${id}`}>Cancel</Link><SubmitButton>Save and rebuild preview</SubmitButton></div></section>
    </form>
  </main>;
}
