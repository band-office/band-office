# Band Office Documentation

Start with the [main README](../README.md) for the product overview and the current releases.

## Start here

- [Download Band Office Desktop](./getting-started/DOWNLOAD.md)
- [Five-minute first-run guide](./getting-started/FIRST_RUN.md)
- [Migrate from CutTime](./getting-started/CUTTIME_MIGRATION.md)
- [Understand where data goes](./getting-started/DATA_FLOW.md)
- [Compatibility and migration notes](./getting-started/COMPATIBILITY.md)

## Product

- [Roadmap](./product/ROADMAP.md)
- [CutTime replacement matrix](./product/CUTTIME_REPLACEMENT_MATRIX.md)
- [Product decisions](./product/DECISIONS.md)

## District deployment

- [Server deployment guide](./deployment/SERVER_DEPLOYMENT.md)
- [Portal activation](./deployment/PORTAL_ACTIVATION.md)
- [Backup and restore](./deployment/SERVER_BACKUP_RESTORE.md)
- [Server upgrades](./deployment/SERVER_UPGRADE.md)
- [macOS signing setup](./deployment/MACOS_SIGNING.md) (maintainers)

## Release evidence

- [Current status](./release/CURRENT_STATUS.md)
- [Release channels](./release/RELEASE_CHANNELS.md)
- [Security checklist](./release/SECURITY_CHECKLIST.md)
