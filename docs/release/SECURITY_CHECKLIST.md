...
**Release state:** public source; Developer ID-signed and Apple-notarized Desktop `v0.1.0-alpha.14` prerelease; district-operated Server `v0.1.0-server-alpha.4` prerelease
...
- [x] A pinned CodeQL `security-extended` workflow analyzes JavaScript and TypeScript on pull requests, `main`, and a weekly schedule. The first full `main` analysis surfaced and drove remediation of two high file-system race findings and one medium network-to-file logging finding; Desktop restore intake now uses a hard-capped file-handle read, Server verification inspects the exact bytes read, and worker diagnostics are fixed strings. The protected-`main` ruleset blocks CodeQL analysis errors or high-and-higher security alerts (`.github/workflows/codeql.yml`, [alerts 1-3](https://github.com/band-office/band-office/security/code-scanning), [ruleset `19774425`](https://github.com/band-office/band-office/rules/19774425)).
...
- [x] Publish the Band Office Server image for `linux/amd64` and `linux/arm64` at `ghcr.io/band-office/band-office-server@sha256:35b05d56032f68d8c04f9feb5d4b25a4c7cdcefdf0e734e54072267808f6bbaa`.
...
- [x] Create the immutable `v0.1.0-alpha.3` release tag at accepted commit `f45cbfdb958018ba11d20cd8f3b1211f30531df3`.
- [x] Publish and verify the Intel x64 macOS package through the native `macos-15-intel` acceptance job in `v0.1.0-alpha.3`.
