...
Desktop alpha.14 replaces alpha.13 after a runtime-experience regression. Alpha.13 launched the private local server by spawning the Electron executable in Node mode, which could create a generic `exec` Dock item. Alpha.14 runs that server as an Electron background utility process instead. It also avoids touching macOS secure storage until an encrypted SMTP credential file actually exists, so a normal first launch does not request Keychain access. The packaged-app acceptance test covers fresh startup, historical upgrade, database integrity, and the complete application surface with the corrected process model.
