...
assert.ok(tag, "Pass an alpha tag or set GITHUB_REF_NAME.");
assert.equal(packageJson.private, true, "package.json must remain private; GitHub Releases distribute Desktop.");
assert.equal(packageJson.license, "Apache-2.0", "Desktop alpha source must remain Apache-2.0.");
...
console.log(`Desktop alpha tag verified: ${tag} matches package version ${packageJson.version}.`);
